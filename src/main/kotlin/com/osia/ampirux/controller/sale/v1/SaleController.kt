package com.osia.ampirux.controller.sale.v1
...
class SaleController(
    ...
    @GetMapping("/{uuid}")
    fun getById(@PathVariable uuid: UUID): ResponseEntity< SaleDto> {
        return ResponseEntity.ok(mapper.toDto(service.getById(uuid)))
    }

    @GetMapping("/multiple")
    fun getByMultipleId(@RequestBody uuidList: List<UUID>): ResponseEntity<List< SaleDto>> {
    ...
    @PatchMapping("/{uuid}")
    fun update(@PathVariable uuid: UUID, @RequestBody request: SaleRequest): ResponseEntity<SaleDto > {
        return ResponseEntity.ok(service.update(uuid, request))
    }

    @PatchMapping("/multiple")
    fun updateMultiple(@RequestBody dtoList: List< SaleDto>): ResponseEntity<List<SaleDto > > {
