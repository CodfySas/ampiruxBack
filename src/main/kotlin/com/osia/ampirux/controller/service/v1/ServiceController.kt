package com.osia.ampirux.controller.service.v1
import com.osia.ampirux.dto.OnCreate
import com.osia.ampirux.dto.service.v1.ServiceDto
import com.osia.ampirux.dto.service.v1.ServiceMapper
...
@RestController("services.v1.crud")
@CrossOrigin
...
class ServiceController(
    ...
    @GetMapping
    fun findAll(pageable: Pageable): Page<ServiceDto> {
        return service.findAll(pageable)
    }

    @GetMapping("/filter/{where}")
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String,
        @RequestHeader("barbershop_uuid") barbershopUuid: UUID): Page<ServiceDto> {
    ...
    @GetMapping("/{uuid}")
    fun getById(@PathVariable uuid: UUID): ResponseEntity<ServiceDto> {
        return ResponseEntity.ok(mapper.toDto(service.getById(uuid)))
    }

    @GetMapping("/multiple")
    fun getByMultipleId(@RequestBody uuidList: List<UUID>): ResponseEntity<List<ServiceDto>> {
        return ResponseEntity.ok(service.findByMultiple(uuidList))
    }

    @PostMapping
    fun save(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody request: ServiceRequest): ResponseEntity<ServiceDto> {
        request.barbershopUuid = barbershopUuid;
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody requestList: List<ServiceRequest>): ResponseEntity<List<ServiceDto> > {
        return ResponseEntity(service.saveMultiple(requestList), HttpStatus.CREATED)
    }

    @PatchMapping("/{uuid}")
    fun update(@PathVariable uuid: UUID, @RequestBody request: ServiceRequest): ResponseEntity<ServiceDto> {
        return ResponseEntity.ok(service.update(uuid, request))
    }

    @PatchMapping("/multiple")
    fun updateMultiple(@RequestBody dtoList: List<ServiceDto>): ResponseEntity<List<ServiceDto> > {
