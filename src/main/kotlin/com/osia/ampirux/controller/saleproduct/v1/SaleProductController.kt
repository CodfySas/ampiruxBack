...
class SaleProductController(
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String): Page< SaleProductDto> {
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: SaleProductRequest): ResponseEntity<SaleProductDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< SaleProductRequest>): ResponseEntity<List<SaleProductDto > > {
