...
class ProductCategoryController(
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String): Page< ProductCategoryDto> {
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: ProductCategoryRequest): ResponseEntity<ProductCategoryDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< ProductCategoryRequest>): ResponseEntity<List<ProductCategoryDto > > {
