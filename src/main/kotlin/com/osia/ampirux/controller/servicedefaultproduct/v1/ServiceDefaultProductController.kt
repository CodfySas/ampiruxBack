...
class ServiceDefaultProductController(
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String,
        @RequestHeader("barbershop_uuid") barbershopUuid: UUID): Page<ServiceDefaultProductDto> {
    ...
    @PostMapping
    fun save(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody request: ServiceDefaultProductRequest): ResponseEntity<ServiceDefaultProductDto> {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody requestList: List<ServiceDefaultProductRequest>): ResponseEntity<List<ServiceDefaultProductDto> > {
