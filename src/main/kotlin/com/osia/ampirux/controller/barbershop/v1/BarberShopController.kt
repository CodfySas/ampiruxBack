...
@RestController("barbershop.v1.crud")
@CrossOrigin
@RequestMapping("v1/barber-shops")
@Validated
class BarberShopController(
private val service:BarberShopService,
private val mapper:BarberShopMapper
)  {
    @GetMapping
    fun findAll(pageable: Pageable): Page< BarberShopDto> {
        return service.findAll(pageable)
    }

    @GetMapping("/filter/{where}")
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String,
        @RequestHeader("barbershop_uuid") barbershopUuid: UUID): Page< BarberShopDto> {
    ...
    @GetMapping("/{uuid}")
    fun getById(@PathVariable uuid: UUID): ResponseEntity< BarberShopDto> {
        return ResponseEntity.ok(mapper.toDto(service.getById(uuid)))
    }

    @GetMapping("/multiple")
    fun getByMultipleId(@RequestBody uuidList: List<UUID>): ResponseEntity<List< BarberShopDto>> {
        return ResponseEntity.ok(service.findByMultiple(uuidList))
    }

    @PostMapping
    fun save(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody request: BarberShopRequest): ResponseEntity<BarberShopDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody requestList: List< BarberShopRequest>): ResponseEntity<List<BarberShopDto > > {
        return ResponseEntity(service.saveMultiple(requestList), HttpStatus.CREATED)
    }

    @PatchMapping("/{uuid}")
    fun update(@PathVariable uuid: UUID, @RequestBody request: BarberShopRequest): ResponseEntity<BarberShopDto > {
        return ResponseEntity.ok(service.update(uuid, request))
    }

    @PatchMapping("/multiple")
    fun updateMultiple(@RequestBody dtoList: List< BarberShopDto>): ResponseEntity<List<BarberShopDto > > {
