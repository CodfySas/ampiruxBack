...
private val mapper:BarberShopMapper
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String): Page< BarberShopDto> {
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: BarberShopRequest): ResponseEntity<BarberShopDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< BarberShopRequest>): ResponseEntity<List<BarberShopDto > > {
