package com.osia.ampirux.controller.client.v1
import com.osia.ampirux.dto.OnCreate
import com.osia.ampirux.dto.client.v1.ClientDto
import com.osia.ampirux.dto.client.v1.ClientMapper
...
class ClientController(
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: ClientRequest): ResponseEntity<ClientDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< ClientRequest>): ResponseEntity<List<ClientDto > > {
        return ResponseEntity(service.saveMultiple(requestList), HttpStatus.CREATED)
    }

    @PatchMapping("/{uuid}")
    fun update(@PathVariable uuid: UUID, @RequestBody request: ClientRequest): ResponseEntity<ClientDto > {
        return ResponseEntity.ok(service.update(uuid, request))
    }

    @PatchMapping("/multiple")
    fun updateMultiple(@RequestBody dtoList: List< ClientDto>): ResponseEntity<List<ClientDto > > {
