...
class SaleServiceController(
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String): Page< SaleServiceDto> {
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: SaleServiceRequest): ResponseEntity<SaleServiceDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< SaleServiceRequest>): ResponseEntity<List<SaleServiceDto > > {
