package com.osia.ampirux.controller.cash.v1
import com.osia.ampirux.dto.OnCreate
import com.osia.ampirux.dto.cash.v1.CashDto
import com.osia.ampirux.dto.cash.v1.CashMapper
...
@RestController("cash.v1.crud")
@CrossOrigin
@RequestMapping("v1/cashs")
@Validated
class CashController(
    private val service: CashService,
    private val mapper: CashMapper
) {
    @GetMapping
    fun findAll(pageable: Pageable): Page< CashDto> {
        return service.findAll(pageable)
    }

    @GetMapping("/filter/{where}")
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String,
        @RequestHeader("barbershop_uuid") barbershopUuid: UUID): Page< CashDto> {
    ...
    @GetMapping("/{uuid}")
    fun getById(@PathVariable uuid: UUID): ResponseEntity< CashDto> {
        return ResponseEntity.ok(mapper.toDto(service.getById(uuid)))
    }

    @GetMapping("/multiple")
    fun getByMultipleId(@RequestBody uuidList: List<UUID>): ResponseEntity<List< CashDto>> {
        return ResponseEntity.ok(service.findByMultiple(uuidList))
    }

    @PostMapping
    fun save(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody request: CashRequest): ResponseEntity<CashDto > {
        request.barbershopUuid = barbershopUuid;
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@RequestHeader("barbershop_uuid") barbershopUuid: UUID, @Validated(OnCreate::class) @RequestBody requestList: List< CashRequest>): ResponseEntity<List<CashDto > > {
        return ResponseEntity(service.saveMultiple(requestList), HttpStatus.CREATED)
    }

    @PatchMapping("/{uuid}")
    fun update(@PathVariable uuid: UUID, @RequestBody request: CashRequest): ResponseEntity<CashDto > {
        return ResponseEntity.ok(service.update(uuid, request))
    }

    @PatchMapping("/multiple")
    fun updateMultiple(@RequestBody dtoList: List< CashDto>): ResponseEntity<List<CashDto > > {
