...
class SaleServiceProductController(
    ...
    fun findAllByFilter(pageable: Pageable, @PathVariable where: String): Page< SaleServiceProductDto> {
    ...
    @PostMapping
    fun save(@Validated(OnCreate::class) request: SaleServiceProductRequest): ResponseEntity<SaleServiceProductDto > {
        return ResponseEntity(service.save(request), HttpStatus.CREATED)
    }

    @PostMapping("/multiple")
    fun saveMultiple(@Validated(OnCreate::class) requestList: List< SaleServiceProductRequest>): ResponseEntity<List<SaleServiceProductDto > > {
