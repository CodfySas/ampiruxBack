package com.osia.ampirux.service.discount.impl
...
class DiscountServiceImpl(
    ...
    private val mapper: BaseMapper<DiscountRequest, Discount, DiscountDto>,
    ...
    @Transactional(readOnly = true)
    override fun findByMultiple(idList: List<UUID>): List<DiscountDto> {
    ...
    @Transactional(readOnly = true)
    override fun findAll(pageable: Pageable): Page<DiscountDto> {
        log.trace("findAll -> pageable: $pageable")
        return repository.findAll(Specification.where(CreateSpec<Discount>().createSpec("")), pageable).map(mapper::toDto)
    }

    @Transactional(readOnly = true)
    override fun findAllByFilter(pageable: Pageable, where: String, barberShopUuid: UUID): Page<DiscountDto> {
        log.trace("findAllByFilter -> pageable: $pageable, where: $where")
        return repository.findAll(Specification.where(CreateSpec<Discount>().createSpec(where)), pageable).map(mapper::toDto)
    }

    @Transactional
    override fun save(request: DiscountRequest, replace: Boolean): DiscountDto {
    ...
    @Transactional
    override fun saveMultiple(requestList: List<DiscountRequest>): List<DiscountDto> {
    ...
    @Transactional
    override fun update(id: UUID, request: DiscountRequest, includeDelete: Boolean): DiscountDto {
    ...
    @Transactional
    override fun updateMultiple(dtoList: List<DiscountDto>): List<DiscountDto> {
