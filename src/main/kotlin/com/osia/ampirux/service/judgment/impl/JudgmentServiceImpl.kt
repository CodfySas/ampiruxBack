package com.osia.ampirux.service.judgment.impl
...
class JudgmentServiceImpl(
    private val repository: JudgmentRepository,
    private val mapper: BaseMapper<JudgmentRequest, Judgment, JudgmentDto>,
    ...
    @Transactional(readOnly = true)
    override fun getById(id: UUID): Judgment {
    ...
    @Transactional(readOnly = true)
    override fun findByMultiple(idList: List<UUID>): List<JudgmentDto> {
    ...
    @Transactional(readOnly = true)
    override fun findAll(pageable: Pageable): Page<JudgmentDto> {
        log.trace("findAll -> pageable: $pageable")
        return repository.findAll(Specification.where(CreateSpec<Judgment>().createSpec("")), pageable).map(mapper::toDto)
    }

    @Transactional(readOnly = true)
    override fun findAllByFilter(pageable: Pageable, where: String): Page<JudgmentDto> {
        log.trace("findAllByFilter -> pageable: $pageable, where: $where")
        return repository.findAll(Specification.where(CreateSpec<Judgment>().createSpec(where)), pageable).map(mapper::toDto)
    }

    @Transactional
    override fun save(request: JudgmentRequest, replace: Boolean): JudgmentDto {
    ...
    @Transactional
    override fun saveMultiple(requestList: List<JudgmentRequest>): List<JudgmentDto> {
    ...
    @Transactional
    override fun update(id: UUID, request: JudgmentRequest, includeDelete: Boolean): JudgmentDto {
    ...
    @Transactional
    override fun updateMultiple(dtoList: List<JudgmentDto>): List<JudgmentDto> {
