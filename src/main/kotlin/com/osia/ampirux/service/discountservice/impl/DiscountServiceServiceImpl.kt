package com.osia.ampirux.service.discountservice.impl
...
@Transactional
@Service
class DiscountServiceServiceImpl(
    private val repository: DiscountServiceRepository,
    private val mapper: BaseMapper<DiscountServiceRequest, DiscountService, DiscountServiceDto>,
    ...
    @Transactional(readOnly = true)
    override fun getById(id: UUID): DiscountService {
    ...
    @Transactional(readOnly = true)
    override fun findByMultiple(idList: List<UUID>): List<DiscountServiceDto> {
    ...
    @Transactional(readOnly = true)
    override fun findAll(pageable: Pageable): Page<DiscountServiceDto> {
        log.trace("findAll -> pageable: $pageable")
        return repository.findAll(Specification.where(CreateSpec<DiscountService>().createSpec("")), pageable).map(mapper::toDto)
    }

    @Transactional(readOnly = true)
    override fun findAllByFilter(pageable: Pageable, where: String, barberShopUuid: UUID): Page<DiscountServiceDto> {
        log.trace("findAllByFilter -> pageable: $pageable, where: $where")
        return repository.findAll(Specification.where(CreateSpec<DiscountService>().createSpec(where)), pageable).map(mapper::toDto)
    }

    @Transactional
    override fun save(request: DiscountServiceRequest, replace: Boolean): DiscountServiceDto {
    ...
    @Transactional
    override fun saveMultiple(requestList: List<DiscountServiceRequest>): List<DiscountServiceDto> {
    ...
    @Transactional
    override fun update(id: UUID, request: DiscountServiceRequest, includeDelete: Boolean): DiscountServiceDto {
    ...
    @Transactional
    override fun updateMultiple(dtoList: List<DiscountServiceDto>): List<DiscountServiceDto> {
