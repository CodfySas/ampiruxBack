...
class NotificationServiceImpl(
    ...
    override fun findAll(pageable: Pageable, school: UUID): Page<NotificationDto> {
        ...
        return notificationRepository.findAll(
            Specification.where(CreateSpec<Notification>().createSpec("", school)),
            pageable
    ...
    override fun findAllByFilter(pageable: Pageable, where: String, school: UUID): Page<NotificationDto> {
        ...
        return notificationRepository.findAll(
            Specification.where(CreateSpec<Notification>().createSpec(where, school)),
            pageable
        ).map(notificationMapper::toDto)
    }
    ...
    override fun save(notificationRequest: NotificationRequest, school: UUID, replace: Boolean): NotificationDto {
        ...
        savedNotification.datetime = LocalDateTime.now(ZoneId.of("America/Bogota"))
    ...
    override fun updateMultiple(notificationDtoList: List<NotificationDto>): List<NotificationDto> {
        ...
        notifications.forEach { notification ->
            notificationMapper.update(
                notificationMapper.toRequest(notificationDtoList.first { it.uuid == notification.uuid }),
                notification
            )
        }
        return notificationRepository.saveAll(notifications).map(notificationMapper::toDto)
    ...
    override fun generateToRole(
        ...
        users.forEach { user ->
            ...
            val notification = NotificationRequest().apply {
                ...
                this.description = notificationRequest.description
            }
            notificationsToSave.add(notification)
        }
        return saveMultiple(notificationsToSave)
    }
}
